...
async function processYear(year: number) {
  ...
  try {
    if (statSync(yearDir).isDirectory()) {
      files = readdirSync(yearDir).filter(f => f.toLowerCase().endsWith(".xml")).map(f => join(yearDir, f));
    }
  } catch (e) {
    console.warn(`Directory not found for ${year}: ${yearDir}`);
    return;
  }
  ...
  for (const file of files) {
    ...
    try {
      ...
      const enTete = doc.EnTeteDocBudgetaire || doc.EnTeteBudget; // Fallback
      ...
      const lines = budget.LigneBudget ? (Array.isArray(budget.LigneBudget) ? budget.LigneBudget : [budget.LigneBudget]) : [];
      ...
      if (rawLoans) {
        ...
        loansArray.forEach((l: any, index: number) => {
          loans.push({
            id: `${year}-${source}-${index}`,
            budget: source as any, // Cast because LoanSchema defines specific enum, assuming types are updated or compatible
        ...
        for (const raw of apcpArray) {
          ...
          if (!apcpsMap.has(id)) {
             ...
             apcpsMap.set(id, {
              ...
              cp_vote_2025: 0, // In this context, it's cp_vote_YEAR
              cp_realise_2025: 0, // cp_realise_YEAR
  ...
  const budgetData: BudgetData = { generated_at: new Date().toISOString(), lines: budgetLines };
  writeFileSync(join(OUTPUT_DIR, `budget_${year}.json`), JSON.stringify(budgetData, null, 2));

  const loanData: LoanData = { generated_at: new Date().toISOString(), loans: loans };
  writeFileSync(join(OUTPUT_DIR, `loans_${year}.json`), JSON.stringify(loanData, null, 2));

  const apcpData: ApcpData = { generated_at: new Date().toISOString(), apcps: Array.from(apcpsMap.values()) };
  writeFileSync(join(OUTPUT_DIR, `apcp_${year}.json`), JSON.stringify(apcpData, null, 2));

  console.log(`Completed ${year}: ${budgetLines.length} budget lines, ${loans.length} loans, ${apcpsMap.size} APCPs.`);
}

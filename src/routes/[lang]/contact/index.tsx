...
const CONTENT_DIR = join(process.cwd(), "src/content/contact");

export const useContactContent = routeLoader$(async ({ params, fail }) => {
  ...
  try {
    ...
    console.error(`Error reading contact file for lang ${lang}:`, error);
  ...
  useTask$(() => {
    config.supportedLocales.forEach((locale) => {
      if (locale.lang === config.defaultLocale.lang) {
        mapping[locale.lang] = `/contact/`;
      } else {
        mapping[locale.lang] = `/${locale.lang}/contact/`;
      }
    });
  });
...
export const head: DocumentHead = () => {
  ...
  return {
    title: t("app.footer.contact") + " - Montpellier Municipales 2026",
  };
};

...
const CONTENT_DIR = join(process.cwd(), "src/content/politique-confidentialite");

export const usePrivacyPolicyContent = routeLoader$(async ({ fail }) => {
  ...
  try {
    ...
    console.error(`Error reading politique-confidentialite file for lang ${lang}:`, error);
    throw fail(404, { errorMessage: "Content not found" });
  }
});

export default component$(() => {
  const data = usePrivacyPolicyContent();
  const mapping = useUrlMapping();

  useTask$(() => {
    config.supportedLocales.forEach((locale) => {
      if (locale.lang === config.defaultLocale.lang) {
        mapping[locale.lang] = `/politique-confidentialite/`;
      } else {
        mapping[locale.lang] = `/${locale.lang}/politique-confidentialite/`;
      }
    });
  });
...
export const head: DocumentHead = () => {
  ...
  return {
    title: t("app.footer.privacyPolicy") + " - Montpellier Municipales 2026",
  };
};
